...
double XBoxController::getJoystick(Side side, Axis axis)
{
    ...
    return handleDeadband((y ? -1.0 : 1.0)*mController->GetRawAxis((left ? 0 : 4) + (y ? 1 : 0)), deadband);
}
...
bool XBoxController::getButton(int button)
{
    ...
    frc::SmartDashboard::PutBoolean("Joystick raw button", mController->GetRawButton(button));
}
