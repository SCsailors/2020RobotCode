...
class GoalTrack {
 ...
 public:
  ...
  static std::shared_ptr<GoalTrack> makeNewTrack(double timestamp, std::shared_ptr<Pose2D> first_observation, int id);
  ...
  void smooth();
  std::shared_ptr<Pose2D> getSmoothedPosition(){smooth(); return mSmoothedPosition;}
